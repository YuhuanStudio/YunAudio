...
## AVFoundation 當輸出路徑：IINA 有那個選項，我們要不要 [待量測]

IINA 設定裡那個 `Core Audio / AVFoundation🧪` 是 **mpv 的 `--ao` 選擇**——播放器
「把聲音推給裝置」用哪一條路。它自己的說明就寫了交換條件：**支援空間音訊，但可能
遇到音訊延遲**。

**對播放器和對路由器，這個交換條件不一樣。**

- 播放器只有一條路：解碼 → 輸出。多幾十毫秒延遲，看片的人不知道。
- 我們是**路由器**，而且賣的就是兩件被量過的事：**位元完全一致**（`MEASUREMENT.md`
  有結果）與**可量測的延遲**（`ProcessingLatency`、每輸出延遲修正）。走 AVFoundation
  的輸出會**依定義終結位元一致**——空間化就是混音與重採樣——而延遲正是唱歌的人
  最不能忍的那一項：**監聽路徑上多 20 ms，人就唱不準了。**

**所以如果做，形狀必須是這樣**（不是照抄 IINA）：

1. **只給輸出，不給監聽。** 伴奏那一路可以空間化；麥克風回送那一路永遠不行。
   混在一起提供是把這個專案的賣點賣掉。
2. **預設關、標成實驗**，跟 IINA 一樣誠實。
3. **兩個數字先量出來才准開**：(a) 開啟後的往返延遲，用現有的量法；(b) 開啟後
   位元一致性**必然失效**——那要在介面上直說，不是藏在說明裡。
4. 真正的 API 是 `AVAudioEngine` + `AVAudioEnvironmentNode` / 空間混音器，或
...
**舞台 moving 這一列只能同場 A/B 比。** 上面那個 150 ms 是那一次量的，換一天、機器上有別
的事在跑，同一份程式碼五次量到的是 **410 / 310 / 268 / 228 / 260 ms**（中位數 268）。所以
判斷「有沒有變慢」的唯一方法是**當場 stash 掉改動、重建、交錯各量五次**，不是拿今天的數字
去對昨天寫下的那個。歌詞景深這一輪就是這樣做的：改動後 **211 / 241 / 254 / 269 / 239**
（中位數 **241 ms**），對照組中位數 268 ms——差距落在展開度以內，**所以是「量不到成本」，
不是「變快了」**。長行逐字版同一支程式是 247 / 276 / 247 ms。

## 兩分鐘就當機 —— 已緩解，根因未定 [本機實測 2026-07-31]

**症狀**：四份當機報告，全部 `EXC_BAD_ACCESS at 0x1e`，全部倒在
`swift_task_isCurrentExecutor`——**runtime 在做「現在是不是 main actor」這個動態檢查時
自己爆掉**。一次在 `LiveCables` 的 `Canvas` 繪製閉包，三次在 20 Hz 輪詢計時器的
`MainActor.assumeIsolated`（`RouterModel.swift:9140`）。後三份**共用同一個 binary
UUID、同一個位置、都在啟動後約兩分鐘**，所以是可重現的。

**排除掉的**：不是「在使用者開著的時候重建 bundle 把執行檔換掉」。那會產生一模一樣的
症狀（執行中的 process 的程式碼分頁是直接從檔案 map 的），所以 `build-app.sh` 現在會
拒絕這麼做——但同一個 commit 之後跑 flow check 兩次都乾淨，而且崩的三份是同一個 UUID。

**`AVAudioEngine` 那條假設已經被推翻。** 當初唯一的間接證據是它在那幾個 process 裡都在，
而且今天以前這個專案從來沒有它。改成開第一首歌才建之後，一個**沒有開過歌**的 session 活過
4:06——看起來像修好了，其實只證明了「沒有引擎就不當機」。**第五次當機**（15:22:29，soak）
發生在 flow check 還在走前面幾節的時候，**歌還沒開、引擎根本沒建出來**，一樣的位址、一樣的
位置。引擎不是原因。
...
| 執行 | 引擎 | 結果 |
|---|---|---|
| 一般啟動（舊版，引擎開機就建）×3 | 有 | 約 **2:00** 倒 |
| flow check ＋ 唱歌 soak（引擎延後建立版） | — | **3:21** 倒，`startPolling` |
| flow check ＋ 唱歌 soak（輪詢改 task 迴圈版） | — | **3:21** 倒，`StatusItemController` |
| 一般啟動（現行版），放著 | 無 | **6:21 活著** |
| 只跑「starting」啟動路由，然後放著五分鐘 | 無 | **5:00 活著** |
| 最小重現（純 AppKit ＋ 每秒二十次動態檢查） | — | **5:00 活著** |

**兩件事因此可以確定：**

1. **一般啟動那條路好轉了。** 舊版一般啟動固定 2:00 倒；現行版放著 6:21 沒事。引擎延後建立
   與輪詢改成靜態隔離，兩個加起來對這條路是有效的。
2. **光是「音訊在跑」不會倒。** 只啟動路由、然後什麼都不碰，五分鐘沒事。

**所以嫌犯縮到：flow check 在「starting」之後、「volume keys」之前做的那些事。** 兩次 3:21
的死都停在 `volume keys`，而那中間那一段在做的是——反覆啟停、切換裝置、套用場景 preset、
開關效果鏈、改取樣率。**某一個拆除路徑把主執行器的狀態帶走了**，之後行程裡任何一次動態檢查
都爆，這解釋了為什麼兩次死在完全不同的地方。

**下一步（還沒做）**，照可能性排：**下一步（還沒做）**，照可能性排：

1. **對分。** 用 `YUNAUDIO_FLOWCHECK_ONLY=<某一節>` ＋ `YUNAUDIO_SOAK_AFTER=300` 一節一節
   走「starting」到「volume keys」之間那幾節：切換裝置、套 preset、換取樣率、效果鏈熱抽換。
   哪一節跑完之後放著會倒，就是它。每一次是五分鐘，十幾次就問完了。
2. 找到之後，看它的**拆除**路徑——最可能是某個東西在被拆掉的時候，把 Swift runtime 對主執行
   器的認知一起帶走了。
...
**剩下的 22 條不是這一輪造成的。** 把這一輪的**起點** `c6bfbe7` 開成獨立 worktree、單獨
建置、跑同一份 flow check：**26 條失敗**，而且**第一條根失敗一模一樣**——

> `the IO cycle counter advances after the input back from Razer Seiren V2 X with a monitor changed`
> （`cycles 24 剛剛，24 一會兒之後`）

現在的樹反而比起點少四條。所以這批紅的是**環境**，不是回歸；最可能是這台機器當下的裝置組合
（Razer Seiren V2 X ＋ 拿一個螢幕當監聽輸出、三個顯示器、BlackHole 都在），也可能是今天連續
跑了太多次把裝置留在某個狀態。

**要注意的是：這代表 flow check 目前不能當作「全綠才算過」的閘門用**——在弄清楚那個根失敗
之前，判斷回歸的方法是**跟基準線比條數與比對失敗清單**，不是看它有沒有全綠。

**下一步**：從那條根查起——`Razer Seiren V2 X` 當輸入、某個螢幕當監聽輸出的組合下，重建之後
IO 週期計數器不前進，但同一時刻 destination bus 量得到 −6.0 dBFS（音訊真的在流）。所以是
**計數器沒被更新**，不是音訊沒動。
