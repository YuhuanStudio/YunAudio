...
### 一個起不來的監聽輸出會把整條路由拖下來 [本機實測]

`direct monitoring` 那一節連續三次跑出 `1 → 0 routes on PG32UCDM` —— 把一台螢幕的
音訊端點指定成監聽輸出，整條路由就停了，不只是第二路混音。監聽是**附加**的輸出，
送給對面的那一路不該跟著死。

比較的組別在同一台機器上：同一節指到 `ASUS PG32UQ` 是 `1 → 3 routes`，一切正常。
差別只在挑到哪一台螢幕。

程式面該怎麼修是清楚的，跟「starting」那一節對目的地做的事一樣：`start` 失敗而當時
掛著監聽時，把監聽拿掉再起一次，並且說出來（要一條新的 `loc()` 字串）。**還沒修，
因為斷言不了** —— 那兩台螢幕在這次工作期間從裝置清單上消失了，而目前接著的裝置沒有
一個起不來。要修就要先能重現：接上那台螢幕，把它選成監聽，斷言 `isRunning` 仍為真
且 `lastError` 說明監聽被放棄了。

flow check 那一邊已經修了：那一節現在會把錯誤講出來，並在路由被它弄倒時重新起來，
所以它只花一個失敗，而不是後面三十節。

### 真的在播的音樂被聽成低了五度的調 [本機實測]
...
最像的嫌疑是分析取的是**處理過之後**的訊號：那次跑的鏈裡有人聲隔離、等化器和閘門，
拿人聲隔離去處理音樂本來就會把頻譜改掉。下一步是把實際的 chroma 印出來對照，而不是
繼續從結果反推。
