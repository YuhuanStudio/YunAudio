...
**唯一的間接證據**：`AVAudioEngine` 在那四個 process 裡都在，今天以前這個專案從來沒有
它，而且它原本是 `RouterModel` 的儲存屬性——開 app 就建，不管有沒有人要播檔案。

**做的事**：改成開第一首歌才建。**量到的**：舊版三次都在約 **2:00** 倒；新版跑到
**4:06** 仍然活著（觀察到 4:12，沒有產生新的當機報告）。

**但這只證明了「沒有引擎就不當機」**——那一輪從頭到尾沒有人開過歌，所以引擎根本沒被
建出來。**還沒測的是：開了一首歌之後，兩分鐘會不會又倒。** 那是下一件要做的事，而且
在測出來之前，不要把「已修」寫在任何地方。

**如果會**：那條路是把 20 Hz 輪詢上的 `MainActor.assumeIsolated` 拿掉——每秒二十次的
動態檢查，在這個 OS 上既然不可靠就不要用，改成一個常駐的 `@MainActor` task 迴圈（靜態
隔離，沒有動態檢查，也不用每一格配一個 Task）。那條路徑有量過的閒置成本與
`BackgroundResourceTests` 盯著，所以改完要重新量。
