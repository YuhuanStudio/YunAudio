...
而**同一段的下一條斷言量到 destination bus −6.0 dBFS**——音訊確實在流。所以問題從來不是
音訊，是**讀數**。

`RoutingEngine.cycleCount` 對三件不同的事都回答 `0`：路由沒在跑、cell 剛被釋放換新、
以及**鎖被引擎佇列拿著**。那個鎖是用 `try` 拿的（等它會讓介面凍結到 `coreaudiod` 高興
為止），所以**讀不到是常態，不是例外**。有十六個地方拿「改變前的數字」去比「改變後的
單一次讀數」，一次撞上鎖就報成路由停擺。

**修法**兩層：

1. `cycleCountIfKnown` 會說「我不知道」（`nil`），`cycleCount` 保留給只要一個數字顯示的
   地方。
2. 十六處合併成一個 `checkAudioIsFlowing`，問的是真正的意思：**兩次連續的「答案」是否
   遞增**。這同時免疫兩個陷阱——重建把 cell 換掉、計數從零重來（這個坑在這個檔案裡有三
   段註解記錄踩過），因為新 cell 一樣從零往上數；以及撞鎖，因為沒有答案的那次直接跳過而
   不是拿去比。真的需要數字本身的兩處（都在偵測「重啟」，也就是計數往回走）用
   `cycleAnswer` 稍等一個答案，讀不到就報「讀不到」——那跟「停擺」是兩種不同的失敗，需要
   不同的修法，不能長得一樣。
