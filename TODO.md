...
**「3:21 是進度不是時間」——這個我寫錯了，更正。** 回頭把今天所有執行排在一起看：

| 執行 | 長度 | 結果 |
|---|---|---|
| flow check 限定一節，沒有 soak | 128 s | **跑完** |
| flow check 限定一節，soak 200 s | 到 3:21 | **倒** |
| flow check 限定一節，soak 240 s | 到 3:21 | **倒** |
| 一般啟動，沒有 flow check，放著 | 4:12+ | **活著**（我手動停的）|

跑完的那次不是因為它避開了什麼，是因為它**還沒跑到 200 秒就結束了**。所以三分二十一秒
比較像是**行程在有音訊活動的情況下的一個時鐘門檻**，不是某一節的位置。而「一般啟動活過
4:12」那次是唯一的反例——那次**完全沒有啟動音訊裝置**，app 是閒著的。

**所以現在最合理的說法**：要有音訊在跑，過了大約三分二十秒，動態 main-actor 檢查開始爆。

**下一步（還沒做）**，照可能性排：

1. **先分開「有音訊」與「沒音訊」。** 把 app 放著跑六分鐘、什麼都不啟動（上次只跑到
   4:12 就被我停掉了，那不算跑完）；再把路由啟動之後放著跑六分鐘、其他什麼都不做。如果
   只有第二個會倒，那嫌犯就是「音訊在跑」這件事本身，而不是 flow check 的任何一節。
2. 然後才是縮到哪一段音訊工作上——啟停裝置、切換裝置、套 preset 各自單獨放著跑。
