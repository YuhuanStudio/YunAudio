...
final class RouterModel: ScriptTarget {
    ...
    private func askThePlayer() {
        ...
        guard let position = NowPlaying.position(preferring: lastPlayer) else {
            lastPlayer = nil
            trackClock.stop()
        ...
        // The position was true somewhere inside the round trip, and its middle
        // is the unbiased guess. Anchoring at the moment the answer arrived
        // would put every extrapolation half a round trip — ten milliseconds —
        // behind the music, permanently and in the same direction.
        let middle =
            (Double(before) + Double(DispatchTime.now().uptimeNanoseconds)) / 2e9
        lastPlayer = position.application

        // The metadata is four more Apple events and 40 ms, so it is asked for
        // only when the song has actually changed. `id of current track` is one
        // event and 3.9 ms, which is what makes that decidable at all.
        let changed =
            nowPlaying == nil || nowPlaying?.application != position.application
            || (!position.identity.isEmpty && position.identity != nowPlaying?.identity)
        if changed {
            adopt(NowPlaying.track(from: position.application))
        }
        if var track = nowPlaying, track.isPlaying != position.isPlaying {
            track.isPlaying = position.isPlaying
            nowPlaying = track
        }
