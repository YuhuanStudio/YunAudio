...
驗收關卡與其底下每一個探測工具。README 列出各步驟；本頁記載每一步確立了什麼，以及
它無法確立什麼。
...
它會編譯、強制執行 strict 格式檢查、跑測試、檢查每一條使用者看得到的字串都經過翻譯
層、組裝 bundle 並證明建置目錄藏起來後仍能載入所有資源、離屏繪製每個面板、拍下每個
分頁的真實視窗照片，並且在 `--full` 時，用 release 建置證明路徑位元精確、以及即時
執行緒不做任何配置。
...
時鐘鎖定，在那個尺度上全都是看不見的，而它們每一個都會毀掉這個專案存在的目的。下面是
使用真實驅動的歷史六分鐘記錄，保留的是可重現的量測形狀，不是目前版本證據。較新的記錄是
0.42% 與 7.1 MB；下一次硬體 gate 必須用一份同時包含 revision、OS build、硬體、frame size
與 topology 的 manifest 取代兩次記錄：
...
375.0 剛好就是 48000/128。在那次歷史記錄中，每分鐘 4 kB 是配置器的雜訊而不是成長 ——
結束時的足跡比自己的中點還低 —— 而程序坐在 4.7 MB。這一項會判定失敗的條件是：記憶體每小時爬超過
...
最後一張照片標記完成後，應用程式必須在五秒內正常結束。PNG 已存在，不會把 CoreAudio
拆除階段的卡死變成成功的截圖。

**`--verify`** 把建好的 app 複製到別的地方、把建置目錄移到拿不到的位置，再執行專用的
資源探測。SwiftPM 的 `Bundle.module` 會退回去找建置目錄，所以一個從來沒把資源 bundle
複製進去的 app，在建置它的那台機器上完美運作，在其他每一台上一啟動就死 ——
`could not load resource bundle`。除了把建置目錄拿走，沒有任何辦法分辨這兩者。探測不會
建立 router，也不會碰音訊硬體；成功、失敗或遭中斷時，script 都會復原建置目錄。
