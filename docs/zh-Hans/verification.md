...
它会编译、强制执行 strict 格式检查、跑测试、检查每一条用户可见的字符串都经过翻译层、
组装 bundle 并证明隐藏构建目录后仍能加载所有资源、离屏渲染每个面板、拍下每个标签页的
真实窗口照片，并且在 `--full` 时，用 release 构建证明路径比特精确、以及实时线程不做
任何分配。
...
时钟锁定，在那个尺度上全都是看不见的，而它们每一个都会毁掉这个项目存在的目的。下面是
使用真实驱动的历史六分钟记录，保留的是可重现的测量形状，不是当前版本证据。较新的记录是
0.42% 与 7.1 MB；下一次硬件 gate 必须用一份同时包含 revision、OS build、硬件、frame size
与 topology 的 manifest 取代两次记录：
...
375.0 恰好就是 48000/128。在那次历史记录中，每分钟 4 kB 是分配器的噪声而不是增长 ——
结束时的占用比自己的中点还低 —— 而进程稳定在 4.7 MB。这一项判定失败的条件是：内存每小时爬升超过
...
最后一张照片标记完成后，应用程序必须在五秒内正常退出。PNG 已经存在，不会把 CoreAudio
拆除阶段的卡死变成成功的截图。

**`--verify`** 把构建好的 app 复制到别处、把构建目录移到拿不到的位置，再运行专用的
资源探测。SwiftPM 的 `Bundle.module` 会退回去找构建目录，所以一个从来没把资源 bundle
复制进去的 app，在构建它的那台机器上完美运行，在其他每一台上一启动就死 ——
`could not load resource bundle`。除了把构建目录拿走，没有任何办法区分这两者。探测不会
构建 router，也不会接触音频硬件；成功、失败或被中断时，脚本都会恢复构建目录。
